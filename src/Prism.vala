...
public class Prism : Window {
	...
	public Prism() {
        ...
        /* Get window width-height by every window size signal */
        this.configure_event.connect ((event) => {
            url_bar.set_max_width_chars(event.width / 12);
            return false;
        });
    ...
    private void create_widgets() {
        ...
        secure_button.set_halign(Align.START);

        incognito_button.set_halign(Align.START);

		headerBar.pack_start(_prism_button);
        headerBar.pack_start(back_button);
        headerBar.pack_start(forward_button);
        headerBar.pack_start(url_bar);

        headerBar.pack_end(secure_button);
